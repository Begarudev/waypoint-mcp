...
Submission for the Waypoint Learning founding-engineer challenge. The original challenge prompt is preserved at [CHALLENGE.md](CHALLENGE.md). The bundled `iep` and `lesson` PDFs at the repo root are the challenge fixtures; the server reads them at runtime.
...
- **11 operative rules** and an **8-section output contract** in `server/src/prompts.ts`.
...
The IEP is parsed into named sections in `server/src/data.ts`. The prompt does not treat every section equally.
...
`server/examples/jasmine_community_lesson.md`
...
`server/examples/marcus_fractions_lesson.md`
...
`server/examples/jasmine_quick_accommodations.md` is intentionally not a full packet. It demonstrates the shorter `/quick_accommodations` shape.

## Eval Harness

`npm run eval` is deterministic and offline.

It measures the shipped markdown packets against `server/eval/rubric.ts`, which wraps `lintModificationPacket` from `server/src/lint.ts`. The same lint engine backs the MCP tool `waypoint_lint_packet`.
...
```text
.
├── README.md           (this file — submission docs)
├── CHALLENGE.md        (original Waypoint challenge prompt)
├── iep                 (challenge fixture, PDF — Jasmine Bailey IEP)
├── lesson              (challenge fixture, PDF — Toby Lowe "community" essay)
├── index.html          (from upstream challenge repo)
└── server/             (the MCP server — submission lives here)
    ├── package.json
    ├── package-lock.json
    ├── tsconfig.json
    ├── data/
...
Runtime note: `server/src/data.ts` reads the root bundled PDFs (`./iep` and `./lesson` at the repo root) for the Jasmine/community fixtures. The matching `server/data/iep-jasmine.txt` and `server/data/lesson-community.txt` files are inspection artifacts, not the runtime source for those two records.
